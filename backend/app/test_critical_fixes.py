...
async def run_tests():
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test/api/v1") as client:
        ...
        login_res = await client.post(
            "/auth/login",
            json={"email": "aseel@alphapromena.com", "password": "Sales123!"}
        )
        ...
        saleh_login = await client.post(
            "/auth/login",
            json={"email": "saleh@alphapromena.com", "password": "Sales123!"}
        )
        ...
        abdullah_login = await client.post("/auth/login", json={"email": "abdallah@alphapromena.com", "password": "Manager123!"})
        ...
        qusai_login = await client.post("/auth/login", json={"email": "qusai@alphapromena.com", "password": "TeamLead123!"})
