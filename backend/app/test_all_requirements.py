...
async def run_verification():
    ...
    async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url="http://test/api/v1") as client:
        ...
        login_res = await client.post("/auth/login", json={"email": "abdullah@alphapromena.com", "password": "Manager123!"})
        if login_res.status_code != 200:
            login_res = await client.post("/auth/login", json={"email": "abdallah@alphapromena.com", "password": "Manager123!"})
        ...
        saleh_res = await client.post("/auth/login", json={"email": "saleh@alphapromena.com", "password": "Sales123!"})
        ...
        aseel_res = await client.post("/auth/login", json={"email": "aseel@alphapromena.com", "password": "DataOps123!"})
        ...
        async with AsyncSessionLocal() as session:
            ...
            if q_user:
                q_user.password_hash = hash_password("TeamLead123!")
                q_user.is_locked = False
                q_user.login_attempts = 0
                await session.commit()
                
        qusai_res = await client.post("/auth/login", json={"email": "qusai@alphapromena.com", "password": "TeamLead123!"})
