...
from scripts.migrations import run_role_migrations
...
async def run_verification():
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test/api/v1") as client:
        ...
        login_res = await client.post("/auth/login", json={"email": "qusai@alphapro.com", "password": "<set-password>"})
        ...
        mgr_login = await client.post("/auth/login", json={"email": "manager@alphapro.com", "password": "<set-password>"})
        ...
        user_login = await client.post("/auth/login", json={"email": "saleh@alphapro.com", "password": "<set-password>"})
